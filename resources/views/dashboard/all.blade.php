...
@section('scripts')
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            db.collection('users').orderBy('createdAt').where("role", "==", "customer").get().then(
                (snapshot) => {
                    jQuery("#users_count").empty();
                    jQuery("#users_count").append(snapshot.docs.length);
                });

            db.collection('users').orderBy('createdAt').where("role", "==", "driver").get().then(
                (snapshot) => {
                    jQuery("#driver_count").empty();
                    jQuery("#driver_count").append(snapshot.docs.length);
                });

            db.collection('vendors').where('title', '!=', '') 
            .get().then(
                (snapshot) => {
        ...
        async function getTotalEarnings() {
            ...
            await db.collection('vendor_orders').where('status', 'in', ["Order Completed"]).get().then(async function (orderSnapshots) {
                ...
                //get ondemand service earnings
                var ondemand_earnings = await getTotalEarningsOndemand().then(function (obj) {
                    return obj;
                })
                if (ondemand_earnings.totalEarning && ondemand_earnings.adminCommission) {
                    totalEarning = totalEarning + parseFloat(ondemand_earnings.totalEarning);
                    adminCommission = adminCommission + parseFloat(ondemand_earnings.adminCommission);
                }
        ...
        async function getTotalEarningsOndemand() {
            ...
            await db.collection('provider_orders').where('status', '==', "Order Completed").get().then(async function (orderSnapshots) {
                var paymentData = orderSnapshots.docs;
                ...
                paymentData.forEach((order) => {
                    ...
                    price = parseFloat(orderData.provider.price);
                    minprice = parseFloat(orderData.provider.price);
                    discount = orderData.discount;
