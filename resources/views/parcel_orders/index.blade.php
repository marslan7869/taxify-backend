...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        database.collection('parcel_orders').get().then((snapshot) => {
            jQuery("#order_count").empty();
            jQuery("#order_count").text(snapshot.docs.length);
        });

        database.collection('parcel_orders').where('status', 'in', ["Order Placed"]).get().then((snapshot) => {
            jQuery("#placed_count").empty();
            jQuery("#placed_count").text(snapshot.docs.length);
        });

        database.collection('parcel_orders').where('status', 'in', ["Order Accepted"]).get().then((snapshot) => {
            jQuery("#accepted_count").empty();
            jQuery("#accepted_count").text(snapshot.docs.length);
        });

        database.collection('parcel_orders').where('status', 'in', ["Order Completed"]).get().then((snapshot) => {
    ...
    async function buildHTML(val) {
        ...
        if (checkDeletePermission) {
            ...
            action = action + '<a id="' + val.id + '" class="delete-btn" name="delete-btn" href="javascript:void(0)"><i class="mdi mdi-delete"></i></a>';
